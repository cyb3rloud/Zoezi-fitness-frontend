...
class App extends React.Component {
  ...
  render() {
    ...
    const filteredItems = this.state.items.filter((item) =>
      item.movie.toLowerCase().includes(this.state.filter.toLowerCase())
    );
          ...
          <div theme={theme}>
            <div maxWidth="xd">
              <header className="App-header">
                <div variant="h2" component="div"></div>
                <div variant="subtitle1"></div>
              </header>
              ...
              <Switch>
                ...
                <Route
                  exact
                  path="/edit"
                  render={() => <EditForm currentUser={this.state.currentUser} patchInfo={this.patchInfo} />}
                />

                <Route exact path="/users" render={() => <Account currentUser={this.state.currentUser} />} />

                <Route exact path="/checkout" render={() => <Checkout currentUser={this.state.currentUser} />} />

                <Route exact path="/thanks" render={() => <Thanks currentUser={this.state.currentUser} />} />
